...
import { Card, Button, Row, Col, Typography, Modal, Divider, Tag, Tooltip, Dropdown, Menu, Empty, Input, Space } from 'antd';
import { PlusOutlined, EditOutlined, CopyOutlined, DeleteOutlined, EllipsisOutlined, SearchOutlined } from '@ant-design/icons';
...
const TaskOrchestrationPage: React.FC = () => {
  ...
  return (
    <>
            ...
            {filteredWorkflows.map(workflow => (
              <Col key={workflow.id} xs={24} sm={12} md={8} lg={6}>
                <Card
                  ...
                  actions={[
                    <Tooltip title="编辑">
                      <EditOutlined key="edit" onClick={() => handleEditWorkflow(workflow)} />
                    </Tooltip>,
                    <Tooltip title="复制">
                      <CopyOutlined key="copy" onClick={() => handleDuplicateWorkflow(workflow)} />
                    </Tooltip>,
                    <Tooltip title="删除">
                      <DeleteOutlined key="delete" onClick={() => handleDeleteConfirm(workflow.id)} />
                    </Tooltip>,
                  ]}
                >
                  <div 
                  ...
                  >
                    ...
                    <Text type="secondary" ellipsis={{ rows: 2, tooltip: workflow.description }}>
      ...
      {/* 任务流编辑器 */}
      <Modal
        title={isCreateMode ? "创建新任务流" : "编辑任务流"}
        open={isEditorVisible}
        onCancel={handleCloseEditor}
        footer={null}
        width="90%"
        style={{ top: 20 }}
        bodyStyle={{ height: 'calc(90vh - 108px)', padding: 0 }}
      >
        <FlowEditor 
