...
// 类型转换函数 - 服务端到前端共享类型
function toSharedPromptTemplate(template) {
    return {
        id: template.id,
        title: template.name,
...
// 类型转换函数 - 前端共享类型到服务端
function toServerPromptTemplate(template) {
    return {
        id: template.id,
        name: template.title,
        content: template.content,
...
// 类型转换函数 - 服务端到前端共享类型
function toSharedPromptCategory(category) {
    return {
        id: category.id,
        name: category.name,
...
// 类型转换函数 - 前端共享类型到服务端
function toServerPromptCategory(category) {
...
// 类型转换函数 - 服务端到前端共享类型
function toSharedPromptBinding(binding) {
    return {
...
// 类型转换函数 - 前端共享类型到服务端
function toServerPromptBinding(binding) {
    return {
        id: (0, uuid_1.v4)(), // 新建绑定时生成ID
...
// 模拟服务端提示模板分类数据
const mockServerPromptCategories = [
    {
        id: '1',
        name: '通用',
...
// 前端可用的模拟提示模板分类数据
const mockPromptCategories = mockServerPromptCategories.map(category => toSharedPromptCategory(category));
// 模拟服务端提示模板数据
const mockServerPromptTemplates = [
    {
        id: '1',
        name: '通用知识问答',
...
// 前端可用的模拟提示模板数据
const mockPromptTemplates = mockServerPromptTemplates.map(template => toSharedPromptTemplate(template));
// 模拟服务端提示模板绑定数据
const mockServerPromptBindings = [
    {
        id: '1',
        prompt_id: '1',
        assistant_id: 'ast1',
...
// 前端可用的模拟提示模板绑定数据
const mockPromptBindings = mockServerPromptBindings.map(binding => toSharedPromptBinding(binding));
class PromptTemplateService {
    ...
    async getPromptTemplates(filters = {}) {
        if (this.useMock) {
            logger_1.logger.info('使用模拟数据获取提示模板列表');
            // 使用服务端模拟数据
            ...
            if (filters.tags && filters.tags.length > 0) {
                filteredTemplates = filteredTemplates.filter(t => filters.tags.some(tag => t.tags.includes(tag)));
            }
            if (filters.search) {
        ...
        try {
            logger_1.logger.info('从后端API获取提示模板列表');
            // 构建查询参数
            ...
            const response = await axios_1.default.get(`${this.apiUrl}/api/prompt-templates?${params.toString()}`);
            // 将服务端类型转换为前端共享类型
            ...
            logger_1.logger.error('获取提示模板列表失败:', error);
    ...
    async getPromptTemplateById(id) {
        if (this.useMock) {
            logger_1.logger.info(`使用模拟数据获取提示模板详情, ID: ${id}`);
        ...
        try {
            logger_1.logger.info(`从后端API获取提示模板详情, ID: ${id}`);
            const response = await axios_1.default.get(`${this.apiUrl}/api/prompt-templates/${id}`);
            // 转换为前端共享类型
            ...
            logger_1.logger.error(`获取提示模板详情失败, ID: ${id}:`, error);
    ...
    async createPromptTemplate(template) {
        if (this.useMock) {
            logger_1.logger.info(`使用模拟数据创建提示模板, 名称: ${template.title}`);
            const now = new Date().toISOString();
            // 转换为服务端类型
        ...
        try {
            logger_1.logger.info(`向后端API创建提示模板, 名称: ${template.title}`);
            // 转换为服务端类型
            const serverTemplate = toServerPromptTemplate({
                ...template,
                id: '',
            ...
            const response = await axios_1.default.post(`${this.apiUrl}/api/prompt-templates`, serverTemplate);
            // 将服务端响应转换为前端类型
            return toSharedPromptTemplate(response.data);
        }
        catch (error) {
            logger_1.logger.error(`创建提示模板失败:`, error);
    ...
    async updatePromptTemplate(id, template) {
        if (this.useMock) {
            logger_1.logger.info(`使用模拟数据更新提示模板, ID: ${id}`);
        ...
        try {
            logger_1.logger.info(`向后端API更新提示模板, ID: ${id}`);
            // 首先获取现有模板
            const currentResponse = await axios_1.default.get(`${this.apiUrl}/api/prompt-templates/${id}`);
            ...
            // 发送更新请求
            const response = await axios_1.default.put(`${this.apiUrl}/api/prompt-templates/${id}`, serverTemplate);
            // 将响应转换为前端类型
            return toSharedPromptTemplate(response.data);
        }
        catch (error) {
            logger_1.logger.error(`更新提示模板失败, ID: ${id}:`, error);
            if (axios_1.default.isAxiosError(error) && error.response?.status === 404) {
                return null;
            }
    ...
    async deletePromptTemplate(id) {
        if (this.useMock) {
            logger_1.logger.info(`使用模拟数据删除提示模板, ID: ${id}`);
            const index = mockServerPromptTemplates.findIndex(t => t.id === id);
            if (index === -1) {
                return false;
            }
        ...
        try {
            logger_1.logger.info(`向后端API删除提示模板, ID: ${id}`);
            await axios_1.default.delete(`${this.apiUrl}/api/prompt-templates/${id}`);
            return true;
        }
        catch (error) {
            logger_1.logger.error(`删除提示模板失败, ID: ${id}:`, error);
    ...
    async getPromptCategories() {
        if (this.useMock) {
            logger_1.logger.info('使用模拟数据获取提示模板分类列表');
            // 将服务端模型转换为前端共享模型
            return mockServerPromptCategories.map(category => toSharedPromptCategory(category));
        }
        try {
            logger_1.logger.info('从后端API获取提示模板分类列表');
            const response = await axios_1.default.get(`${this.apiUrl}/api/prompt-categories`);
            return response.data;
        }
        catch (error) {
            logger_1.logger.error('获取提示模板分类列表失败:', error);
     ...
     * 创建提示模板分类
     */
    async createPromptCategory(category) {
        if (this.useMock) {
            logger_1.logger.info(`使用模拟数据创建提示模板分类, 名称: ${category.name}`);
            const now = new Date().toISOString();
        ...
        try {
            logger_1.logger.info(`向后端API创建提示模板分类, 名称: ${category.name}`);
            const response = await axios_1.default.post(`${this.apiUrl}/api/prompt-categories`, category);
            return response.data;
        }
        catch (error) {
            logger_1.logger.error(`创建提示模板分类失败:`, error);
     ...
     * 更新提示模板分类
     */
    async updatePromptCategory(id, category) {
        if (this.useMock) {
            logger_1.logger.info(`使用模拟数据更新提示模板分类, ID: ${id}`);
            // 使用服务端模型数组
        ...
        try {
            logger_1.logger.info(`向后端API更新提示模板分类, ID: ${id}`);
            const response = await axios_1.default.put(`${this.apiUrl}/api/prompt-categories/${id}`, category);
            return response.data;
        }
        catch (error) {
            logger_1.logger.error(`更新提示模板分类失败, ID: ${id}:`, error);
            if (axios_1.default.isAxiosError(error) && error.response?.status === 404) {
                return null;
            }
    ...
    async deletePromptCategory(id) {
        if (this.useMock) {
            logger_1.logger.info(`使用模拟数据删除提示模板分类, ID: ${id}`);
            // 检查是否有模板使用此分类
        ...
        try {
            logger_1.logger.info(`向后端API删除提示模板分类, ID: ${id}`);
            await axios_1.default.delete(`${this.apiUrl}/api/prompt-categories/${id}`);
            return true;
        }
        catch (error) {
            logger_1.logger.error(`删除提示模板分类失败, ID: ${id}:`, error);
     ...
     * 获取助手的提示模板绑定
     */
    async getAssistantPromptBindings(assistantId) {
        if (this.useMock) {
            logger_1.logger.info(`使用模拟数据获取助手的提示模板绑定, 助手ID: ${assistantId}`);
            // 使用服务端模型过滤
        ...
        try {
            logger_1.logger.info(`从后端API获取助手的提示模板绑定, 助手ID: ${assistantId}`);
            const response = await axios_1.default.get(`${this.apiUrl}/api/assistants/${assistantId}/prompts`);
            // 转换为前端共享类型
            return response.data.map(binding => toSharedPromptBinding(binding));
        }
        catch (error) {
            logger_1.logger.error(`获取助手的提示模板绑定失败, 助手ID: ${assistantId}:`, error);
     ...
     * 创建提示模板绑定
     */
    async createPromptBinding(binding) {
        if (this.useMock) {
            logger_1.logger.info(`使用模拟数据创建提示模板绑定, 提示ID: ${binding.promptId}, 助手ID: ${binding.assistantId}`);
            // 转换为服务端类型
        ...
        try {
            logger_1.logger.info(`向后端API创建提示模板绑定, 提示ID: ${binding.promptId}, 助手ID: ${binding.assistantId}`);
            // 转换为服务端类型
            const serverBinding = toServerPromptBinding({
                ...binding,
            ...
            const response = await axios_1.default.post(`${this.apiUrl}/api/assistants/${binding.assistantId}/prompts`, serverBinding);
            // 转换响应为前端共享类型
            return toSharedPromptBinding(response.data);
        }
        catch (error) {
            logger_1.logger.error(`创建提示模板绑定失败:`, error);
     ...
     * 更新提示模板绑定
     */
    async updatePromptBinding(id, binding) {
        if (this.useMock) {
            logger_1.logger.info(`使用模拟数据更新提示模板绑定, ID: ${id}`);
        ...
        try {
            logger_1.logger.info(`向后端API更新提示模板绑定, ID: ${id}`);
            // 首先获取现有绑定
            const currentResponse = await axios_1.default.get(`${this.apiUrl}/api/prompt-bindings/${id}`);
            ...
            // 发送更新请求
            const response = await axios_1.default.put(`${this.apiUrl}/api/prompt-bindings/${id}`, updatedServerBinding);
            // 转换响应为前端类型
            return toSharedPromptBinding(response.data);
        }
        catch (error) {
            logger_1.logger.error(`更新提示模板绑定失败, ID: ${id}:`, error);
            if (axios_1.default.isAxiosError(error) && error.response?.status === 404) {
                return null;
            }
     ...
     * 删除提示模板绑定
     */
    async deletePromptBinding(id) {
        if (this.useMock) {
            logger_1.logger.info(`使用模拟数据删除提示模板绑定, ID: ${id}`);
            const index = mockServerPromptBindings.findIndex(b => b.id === id);
            if (index === -1) {
                return false;
            }
        ...
        try {
            logger_1.logger.info(`向后端API删除提示模板绑定, ID: ${id}`);
            await axios_1.default.delete(`${this.apiUrl}/api/prompt-bindings/${id}`);
            return true;
        }
        catch (error) {
            logger_1.logger.error(`删除提示模板绑定失败, ID: ${id}:`, error);
