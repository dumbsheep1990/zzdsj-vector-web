...
const Sidebar: React.FC = () => {
    ...
    const renderNavItem = useCallback((item: NavigationItem, isChild: boolean = false, index: number = 0) => {
        ...
        const hasChildren = Boolean(item.children && item.children.length > 0);
        // Explicitly cast to required types to avoid type errors
        const isChildBoolean: boolean = isChild;
        const hasChildrenBoolean: boolean = hasChildren;

        const currentStyle = {
            ...getNavItemStyle(isActive, isChildBoolean, hasChildrenBoolean),
                ...
                >
                    <span style={iconContainerStyle}>
                        <span style={{ display: 'flex', alignItems: 'center' }}>
                            <span style={getIconStyle(isActive, isChildBoolean, hasChildrenBoolean)}>
