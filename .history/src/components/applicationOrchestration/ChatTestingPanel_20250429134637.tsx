...
const ChatTestingPanel: React.FC<ChatTestingPanelProps> = ({
  ...
  const dragHandleElement = useRef<HTMLDivElement | null>(null);
  ...
  useEffect(() => {
    ...
    // Store reference for accessing element later
    dragHandleElement.current = dragHandle;
  ...
  useEffect(() => {
    if (dragHandleElement.current) {
      const innerLine = dragHandleElement.current.querySelector('div');
