...
const ToolOrchestrationStep: React.FC<ToolOrchestrationStepProps> = ({
  ...
  const generateKnowledgeBaseItems = (): OrchestrationItem[] => {
    return selectedKnowledgeBases.map((kb, index) => ({
      ...
      config: kb as unknown as Record<string, unknown>,
      enabled: true,
  ...
  React.useEffect(() => {
    ...
    // 如果没有编排项或者选择的内容发生变化，重新生成
    if (orchestrationItems.length === 0 || 
        allItems.length !== orchestrationItems.length ||
        allItems.some(item => !orchestrationItems.find(existing => existing.id === item.id))) {
      onOrchestrationItemsChange(allItems);
    }
  }, [selectedTools, selectedKnowledgeBases, extensionToolsConfig, orchestrationItems]);
