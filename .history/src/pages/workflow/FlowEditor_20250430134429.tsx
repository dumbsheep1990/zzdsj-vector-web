    ...
    >
      ...
      <div className="editor-header">
        <div className="editor-title">
          <h2>{workflowFormValues.name || (isCreateMode ? "新建工作流" : "编辑工作流")}</h2>
        </div>
        
        <div className="editor-actions">
          <div className="toolbar-section">
            <Tooltip title={historyIndex > 0 ? "撤销" : "没有可撤销的操作"}>
              <Button 
                icon={<UndoOutlined />} 
                onClick={handleUndo} 
            ...
            <Tooltip title={historyIndex < history.length - 1 ? "重做" : "没有可重做的操作"}>
              <Button 
                icon={<RedoOutlined />} 
                onClick={handleRedo} 
            ...
            <Button 
              icon={nodesLocked ? <UnlockOutlined /> : <LockOutlined />}
              onClick={nodesLocked ? handleUnlockNodes : handleLockNodes}
            />
            <Tooltip title={selectedNode ? "删除选中节点" : "请先选择节点"}>
              <Button 
                icon={<DeleteOutlined />} 
                onClick={() => selectedNode && handleDeleteSelectedNode(selectedNode.id)}
          ...
          <div className="toolbar-section toolbar-view-controls">
            <Tooltip title="放大">
