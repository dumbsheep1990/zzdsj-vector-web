...
router.get('/conversations', async (req, res) => {
  try {
    const params = {
      ...
      assistant_id: req.query.assistant_id ? parseInt(req.query.assistant_id as string) : undefined,
...
router.post('/conversations', async (req, res) => {
  try {
    const data: ConversationCreateRequest = req.body;
...
router.put('/conversations/:id', async (req, res) => {
  try {
    ...
    const data: ConversationUpdateRequest = req.body;
    const conversation = await chatService.updateConversation(id, data);
...
router.post('/conversations/:id/restore', async (req, res) => {
  try {
    ...
    const conversation = await chatService.restoreConversation(id);
...
router.post('/chat', async (req, res) => {
  try {
    ...
    if (request.stream) {
      ...
      await chatService.sendMessageStream(request, (chunk) => {
...
router.post('/chat/voice', upload.single('audio_file'), async (req, res) => {
  try {
    ...
    const result = await chatService.voiceChat(request);
...
router.post('/messages/:id/rate', async (req, res) => {
  try {
    ...
    const message = await chatService.rateMessage(messageId, rating, feedback);
...
router.post('/search/messages', async (req, res) => {
  try {
    const request: MessageSearchRequest = req.body;
...
router.post('/voice/speech', async (req, res) => {
  try {
    ...
    const audioBlob = await chatService.textToSpeech(text, voice, speed);
...
router.get('/stats', async (req, res) => {
  try {
    ...
    const stats = await chatService.getConversationStats(dateFrom, dateTo);
...
router.post('/export', async (req, res) => {
  try {
    const request: ConversationExportRequest = req.body;
    ...
    const result = await chatService.exportConversations(request);
...
router.post('/conversations/batch', async (req, res) => {
  try {
    ...
    const result = await chatService.batchOperateConversations(conversation_ids, operation, metadata);
