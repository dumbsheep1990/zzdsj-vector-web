  ...
  return (
    <div className={`agent-node ${selected ? 'selected' : ''}`}>
      ...
      <div className="node-header">
        <div>{data.label}</div>
        <div className="node-icon">{data.config?.icon || 'A'}</div>
      </div>
      
      <div className="node-type">
        {data.type}
        {data.config && 'model' in data.config && data.config.model && (
          <span className="node-model"> • {data.config.model}</span>
        )}
      </div>
      ...
      {/* 新增: 显示步骤信息 */}
      {data.config && 'steps' in data.config && data.config.steps && data.config.steps.length > 0 && (
        <div className="node-steps">
          <div className="section-title">流程步骤</div>
          <div className="steps-list">
            {data.config.steps.map((step, index) => (
      ...
      {/* 新增: 显示能力信息 */}
      {data.config && 'capabilities' in data.config && data.config.capabilities && data.config.capabilities.length > 0 && (
        <div className="node-capabilities">
          <div className="section-title">能力</div>
          <div className="capabilities-list">
            {data.config.capabilities.map((capability, index) => (
...
const ToolNode = ({ data, selected }: { data: NodeData; selected: boolean }) => {
  return (
    <div className={`tool-node ${selected ? 'selected' : ''}`}>
      ...
      <div className="node-header">
        <div>{data.label}</div>
        <div className="node-icon">{data.config?.icon || 'T'}</div>
      </div>
      ...
      {/* 新增: 显示参数信息 */}
      {data.config && 'parameters' in data.config && data.config.parameters && data.config.parameters.length > 0 && (
        <div className="node-parameters">
          <div className="section-title">参数</div>
          <div className="parameters-list">
            {data.config.parameters.map((param: any) => (
      ...
      {/* 新增: 显示功能示例 */}
      {data.config && 'usage_examples' in data.config && data.config.usage_examples && data.config.usage_examples.length > 0 && (
        <div className="node-examples">
          <div className="section-title">使用示例</div>
          <div className="examples-list">
            {data.config.usage_examples.map((example: string, index: number) => (
...
const ApplicationNode = ({ data, selected }: { data: NodeData; selected: boolean }) => {
  return (
    <div className={`application-node ${selected ? 'selected' : ''}`}>
      ...
      <div className="node-header">
        <div>{data.label}</div>
        <div className="node-icon">{data.config?.icon || 'C'}</div>
      </div>
      ...
      {/* 新增: 显示功能列表 */}
      {data.config && 'features' in data.config && data.config.features && data.config.features.length > 0 && (
        <div className="node-features">
          <div className="section-title">功能列表</div>
          <div className="features-list">
            {data.config.features.map((feature: string, index: number) => (
      ...
      {/* 新增: 显示集成方式 */}
      {data.config && 'integrations' in data.config && data.config.integrations && data.config.integrations.length > 0 && (
        <div className="node-integrations">
          <div className="section-title">集成方式</div>
          <div className="integrations-list">
            {data.config.integrations.map((integration: string, index: number) => (
