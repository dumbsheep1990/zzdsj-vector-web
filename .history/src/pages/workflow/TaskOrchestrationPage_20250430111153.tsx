...
const TaskOrchestrationPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // 修复ReactFlow导致的导航问题
    const disableFlowPanePointerEvents = (e: MouseEvent) => {
      // 检查鼠标是否在侧边栏区域
      const sidebar = document.querySelector('.sidebar-container');
      if (sidebar && e.target instanceof Node && sidebar.contains(e.target as unknown as Node)) {
        // 临时禁用ReactFlow面板的点击事件
    ...
    // 添加点击事件监听器
    document.addEventListener('mousedown', disableFlowPanePointerEvents);

    // 清理函数
    return () => {
      document.body.style.overflow = '';
      document.documentElement.style.overflow = '';
      document.removeEventListener('mousedown', disableFlowPanePointerEvents);
    };
  }, []);
