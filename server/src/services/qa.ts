...
const mockQaPairs: Record<string, QaPair[]> = {
  'dataset1': [
    {
      id: 'qp1',
      question: '什么是向量数据库？',
...
class QaService {
  ...
  async getQuestions(assistantId?: string): Promise<Question[]> {
    if (this.useMock) {
      logger.info(`使用模拟数据获取问题列表, 助手ID: ${assistantId || '所有'}`);
      if (assistantId) {
    ...
    try {
      logger.info(`从后端API获取问题列表, 助手ID: ${assistantId || '所有'}`);
  ...
  async getQuestionById(id: string): Promise<{ question: Question, answer: Answer | null } | null> {
    if (this.useMock) {
      logger.info(`使用模拟数据获取问题详情, ID: ${id}`);
    ...
    try {
      logger.info(`从后端API获取问题详情, ID: ${id}`);
      const response = await axios.get(`${this.apiUrl}/api/qa/questions/${id}`);
      return response.data;
    } catch (error) {
      logger.error(`获取问题详情失败, ID: ${id}:`, error);
  ...
  async createQuestion(question: Omit<Question, 'id' | 'createdAt' | 'status'>): Promise<Question> {
    if (this.useMock) {
      logger.info(`使用模拟数据创建问题, 助手ID: ${question.assistantId}`);
    ...
    try {
      logger.info(`向后端API创建问题, 助手ID: ${question.assistantId}`);
      const response = await axios.post<Question>(`${this.apiUrl}/api/qa/questions`, question);
      return response.data;
    } catch (error) {
  ...
  async updateQuestion(id: string, question: Partial<Question>): Promise<Question | null> {
    ...
    try {
      logger.info(`向后端API更新问题, ID: ${id}`);
      const response = await axios.put<Question>(`${this.apiUrl}/api/qa/questions/${id}`, question);
      return response.data;
    } catch (error) {
      logger.error(`更新问题失败, ID: ${id}:`, error);
      if (axios.isAxiosError(error) && error.response?.status === 404) {
        return null;
      }
  ...
  async deleteQuestion(id: string): Promise<boolean> {
    ...
    try {
      logger.info(`向后端API删除问题, ID: ${id}`);
      await axios.delete(`${this.apiUrl}/api/qa/questions/${id}`);
      return true;
    } catch (error) {
      logger.error(`删除问题失败, ID: ${id}:`, error);
      if (axios.isAxiosError(error) && error.response?.status === 404) {
        return false;
      }
  ...
  async getQaPairsByDatasetId(datasetId: string): Promise<QaPair[]> {
    if (this.useMock) {
      logger.info(`使用模拟数据获取数据集问答对, 数据集ID: ${datasetId}`);
      return mockQaPairs[datasetId] || [];
    }
    
    try {
      logger.info(`从后端API获取数据集问答对, 数据集ID: ${datasetId}`);
      const response = await axios.get<QaPair[]>(`${this.apiUrl}/api/qa/datasets/${datasetId}/qa-pairs`);
      return response.data;
    } catch (error) {
      logger.error(`获取数据集问答对失败, 数据集ID: ${datasetId}:`, error);
      throw new Error(`获取数据集问答对失败: ${error}`);
    }
  }
  
  async createQaPair(datasetId: string, qaPair: Omit<QaPair, 'id' | 'datasetId' | 'createdAt'>): Promise<QaPair> {
    if (this.useMock) {
      logger.info(`使用模拟数据创建问答对, 数据集ID: ${datasetId}`);
    ...
    try {
      logger.info(`向后端API创建问答对, 数据集ID: ${datasetId}`);
      const response = await axios.post<QaPair>(`${this.apiUrl}/api/qa/datasets/${datasetId}/qa-pairs`, qaPair);
      return response.data;
    } catch (error) {
      logger.error(`创建问答对失败, 数据集ID: ${datasetId}:`, error);
      throw new Error(`创建问答对失败: ${error}`);
    }
  }
  
  async saveSplitQaPairs(datasetId: string, qaPairs: Omit<QaPair, 'id' | 'datasetId' | 'createdAt'>[]): Promise<QaPair[]> {
    if (this.useMock) {
      logger.info(`使用模拟数据保存拆分问答对, 数据集ID: ${datasetId}`);
      const newQaPairs: QaPair[] = qaPairs.map(qaPair => ({
        id: uuidv4(),
        ...qaPair,
        datasetId,
        createdAt: new Date().toISOString()
    ...
    try {
      logger.info(`向后端API保存拆分问答对, 数据集ID: ${datasetId}`);
      const response = await axios.post<QaPair[]>(`${this.apiUrl}/api/qa/datasets/${datasetId}/split-qa-pairs`, qaPairs);
      return response.data;
    } catch (error) {
      logger.error(`保存拆分问答对失败, 数据集ID: ${datasetId}:`, error);
