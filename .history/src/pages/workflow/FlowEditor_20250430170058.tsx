...
import { Button, Card, Input, Select, Typography, Drawer, Form, message, Modal, Table, Tag, Input as AntInput, Tooltip, Space, Divider, Badge, Collapse, Popover, Switch } from 'antd';
import { 
  UndoOutlined, 
  RedoOutlined, 
  LockOutlined, 
...
const NodeToolbar: React.FC<NodeToolbarProps> = ({ 
  ...
  const getToolbarStyle = () => {
    switch(nodeType) {
      case 'agent':
        return { 
          className: "node-toolbar agent-toolbar",
          btnColor: "#8a5cf6" // 紫色
        };
      case 'tool':
        return { 
          className: "node-toolbar tool-toolbar",
          btnColor: "#0dcaf3" // 蓝色
        };
      case 'application':
        return { 
          className: "node-toolbar application-toolbar",
          btnColor: "#ffae3a" // 橙色
        };
      default:
        return { 
          className: "node-toolbar",
          btnColor: "#1890ff" // 默认蓝色
        };
    }
  };
  
  const { className, btnColor } = getToolbarStyle();
    ...
    <div className={className} onClick={(e) => e.stopPropagation()}>
      <Tooltip title="编辑节点">
        <Button 
          size="small" 
          type="text"
          icon={<EditOutlined style={{ color: btnColor }} />} 
      ...
      <Tooltip title="复制节点">
        <Button 
          size="small" 
          type="text"
          icon={<CopyOutlined style={{ color: btnColor }} />} 
      ...
      <Tooltip title="连接节点">
        <Button 
          size="small" 
          type="text"
          icon={<NodeIndexOutlined style={{ color: btnColor }} />} 
      ...
      {onInfo && (
        <Tooltip title="查看详情">
          <Button 
            size="small" 
            type="text"
            icon={<InfoCircleOutlined style={{ color: btnColor }} />} 
      ...
      {onLock && (
        <Tooltip title="锁定节点">
          <Button 
            size="small" 
            type="text"
            icon={<LockOutlined style={{ color: btnColor }} />} 
...
const mockTools: ToolConfig[] = [
  ...
  { 
    ...
    usage_examples: ['分析客户评论情感', '提取文章关键词'],
    icon: '📝'
  },
  { 
    id: 'tool5', 
    ...
    usage_examples: ['英文翻译成中文', '自动检测语言并翻译'],
    icon: '🌐'
  }
];
...
const NodePanel: React.FC<NodePanelProps> = ({ agents, tools, applications, onAddNode }) => {
  ...
  const renderNodeItem = (item: any, type: 'agent' | 'tool' | 'app') => {
    const itemClass = `node-item ${type}-item`;
    const icon = type === 'agent' ? item.icon || '🤖' : 
                 type === 'tool' ? item.icon || '🔧' : 
                 item.icon || '🖥️';
    ...
    return (
      <div 
        ...
        className={itemClass}
        onClick={() => onAddNode(
          type === 'agent' ? 'agent' : type === 'tool' ? 'tool' : 'application', 
          item
        )}
      >
        <div className="node-item-icon">{icon}</div>
        <div className="node-item-content">
          <div className="node-item-title">{item.name}</div>
  ...
  return (
    <div className="node-panel">
      <div className="node-panel-header">
        <h3 className="node-panel-title">添加节点</h3>
        <Button 
          type="text" 
      ...
      {expanded && (
        <div className="node-panel-content">
          ...
          {(!searchText || filteredAgents.length > 0) && (
            <div className="node-category">
              <div className="node-category-title">
                <RobotOutlined style={{ marginRight: 6 }} /> 智能体
                <Badge count={filteredAgents.length} style={{ backgroundColor: '#8a5cf6' }} size="small" />
              </div>
              <div>
                {filteredAgents.map(agent => renderNodeItem(agent, 'agent'))}
              </div>
            </div>
          )}
          ...
          {(!searchText || filteredTools.length > 0) && (
            <div className="node-category">
              <div className="node-category-title">
                <ToolOutlined style={{ marginRight: 6 }} /> 工具
                <Badge count={filteredTools.length} style={{ backgroundColor: '#0dcaf3' }} size="small" />
              </div>
              <div>
                {filteredTools.map(tool => renderNodeItem(tool, 'tool'))}
              </div>
            </div>
          )}
          ...
          {(!searchText || filteredApps.length > 0) && (
            <div className="node-category">
              <div className="node-category-title">
                <AppstoreOutlined style={{ marginRight: 6 }} /> 应用
                <Badge count={filteredApps.length} style={{ backgroundColor: '#ffae3a' }} size="small" />
              </div>
              <div>
                {filteredApps.map(app => renderNodeItem(app, 'app'))}
              </div>
            </div>
          )}
        </div>
...
const FlowEditor: React.FC<FlowEditorProps> = ({
  ...
  // 在 FlowEditor 组件内部实现新的添加节点方法
  const handleAddNode = useCallback((type: string, config: any) => {
    const newNode = {
      id: `node_${Date.now()}`,
      type,
      data: { 
    ...
    setNodes((nds) => [...nds, newNode]);
    message.success(`已添加${type === 'agent' ? '智能体' : type === 'tool' ? '工具' : '应用'}: ${config.name}`);
  }, [setNodes]);
  ...
  return (
    ...
    >
      {/* 添加默认工具栏，当renderHeaderTools未提供时显示 */}
